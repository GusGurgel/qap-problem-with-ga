def get_qap_total_flux(local_matrix, flux_matrix, objects_allocation):
    """
    Rotonar o fluxo total de uma solução QAP (Quadratic Assignment Problem)

    - **local_matrix**: Matriz de locais
    ...
    if len(local_matrix[0]) != len(flux_matrix[0]):
        raise ValueError("Invalid matrix size")

    if len(local_matrix[0]) != len(objects_allocation):
